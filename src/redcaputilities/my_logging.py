...
def patient_data_directory() -> str:
    ...
    if not patient_data_dir:  # pragma: no cover
        patient_data_dir = r"F:\dbmi.data\redcap_refresh"
...
def setup_logging(log_filename: Union[str, None] = None) -> logging.Logger:
    ...
    if not isinstance(log_filename, str):
        #   If user didn't supply a filename,
        #   create it with the calling module's name.
        #   (Don't use calling >function<,
        #    as it will likely be '__init__', which tells us nothing.)
